import { Tabs, TabList, Tab, TabPanels, TabPanel, Heading, Text, Box, useMediaQuery } from "@chakra-ui/react"
...
const AboutTabs = () => {
  ...
  return (
    <Tabs index={tabIndex} onChange={handleTabsChange} variant="soft-rounded" mt="32px">
      <TabList gap="16px" overflowX="scroll" scrollSnapType="x">
        {ABOUT_TABS.map((tab) => (
          <Box
            key={tab.id}
            transition="all .3s"
            bg={selected.id === tab.id ? "linear-gradient(246.01deg, #FE6569 -8%, rgba(255, 255, 255, 0) 38.07%, rgba(205, 102, 208, 0.74) 72.89%, #563FEB 97.11%)" : "rgba(255, 255, 255, 0.4)"}
            borderRadius="100px"
          >
            <Tab
              w="max-content"
